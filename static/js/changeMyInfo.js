    ...
    methods:{
        ...
        getInfo(){
            ...
            axios.get("/info/",{
                ...
                that.prov = that.userInfo.address.split('-')[0];
                that.city = that.userInfo.address.split('-')[1];
                that.district = that.userInfo.address.split('-')[2]
            }).catch((err)=>{
                console.log(err)
            })
        },
        changeInfo(){
            ...
            if(this.prov && this.city && this.district && this.userInfo.gender && this.userInfo.nick_name &&this.userInfo.desc){
                axios.patch("/info/",{
                    "nick_name":that.userInfo.nick_name,
                    "gender":that.userInfo.gender,
                    "address":that.prov + '-' + that.city + '-' +that.district,
        ...
        changepsw:function () {
            ...
            let that = this;
            if(that.oldpsw.length<6 || that.oldpsw.length > 20 || that.newpsw.length <6 || that.newpsw.length >20 || that.confirmpsw != that.newpsw){
                alert("请输入正确的信息！")
            }else{
                axios.post("/password/",{
                    ...
                    alert("密码修改成功！");
                    location.href = "./changeMyinfo.html"
                }).catch((err)=>{
                    alert(err)
                })
            }
